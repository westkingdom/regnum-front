...
def send_duty_request_email(form_data: Dict[str, Any], user_email: str) -> bool:
    ...
    if USE_SIMULATED_EMAIL:
        ...
        print(f"DEBUG: Calling actual_send_duty_request_email for {user_email}")
        ...
        try:
            ...
            print("ERROR: utils.email.send_duty_request_email not found or import failed.")
            return False
        except Exception as e:
            st.error(f"An error occurred while trying to send the email: {e}")
            print(f"ERROR: Exception during actual_send_duty_request_email call: {e}")
            return False
